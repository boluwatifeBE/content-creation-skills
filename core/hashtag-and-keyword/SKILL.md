...
description: Recommends hashtags for short-form platforms and SEO keywords/tags for long-form platforms, based on the user's actual persona, niche, and the specific piece of content, never invented from a generic template. Use whenever the user asks for hashtags, tags, keywords, wants help with discoverability, asks "what hashtags should I use," or wants a piece optimized to be found on a platform. Requires creator-context to be active for persona, profile, and platform format classifications.
---
...
Confirm `creator-context` is active for persona, niche, and platform format classifications. This skill leans on the persona more than most others in this system, generic terms are the main failure mode to avoid here (see below), so don't proceed without knowing the user's actual field and specialization.
...
## Short-form platforms: hashtags

For platforms classified as short-form (Instagram, X, or a custom platform classified that way):
...
- Recommend a reasonable count per platform (Instagram tends to tolerate more tags than X), and say why that count, rather than just listing a number without explanation
...
## Long-form platforms: SEO keywords and tags

For platforms classified as long-form (LinkedIn, Medium, Substack, or a custom platform classified that way):
...
Present it as a clearly labeled set (broad/niche/community for short-form, tags/keywords for long-form), with a one-line reason for each category, not just a bare list. Invite the user to swap out anything that doesn't fit, they know their actual audience and community better than a generated guess does.
